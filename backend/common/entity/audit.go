...
type AuditAccessRecord struct {
	...
	BizDesc     string `gorm:"column:biz_desc;type:VARCHAR(50);not null" json:"biz_desc"`     // 业务描述
	AuditCode   string `gorm:"column:audit_code;type:VARCHAR(50);not null" json:"audit_code"` // 审核代码
	Status      int    `gorm:"column:status;default:0" json:"status"`                         // 审核状态 0-待审核 1-通过 2-驳回
