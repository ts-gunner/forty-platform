...
function CustomerDetailPage() {
    ...
    useEffect(() => {
        // 实际开发中根据 ID 请求 API
        const data = CUSTOMER_INFO_LIST.find(item => item.key === id) || CUSTOMER_INFO_LIST[0];
        setInfo(data);
    ...
    return (
        <View>
            <View
                className="fixed top-0 flex items-end"
                style={{
            ...
            <ScrollView scrollY className="min-h-screen mesh-gradient pb-20" style={{ paddingTop: `${navBarHeight}px` }}>
                {/* 顶部 Header：公司大标题 */}
                <View className="p-6">
                    <View className="flex justify-between items-start">
                        <View>
                            <Text className="text-2xl font-bold text-gray-800">{info.companyName}</Text>
                            <View className="flex gap-2 mt-2">
                                <AtTag size="small" circle active type="primary">{info.tag}</AtTag>
                                <AtTag size="small" circle>{info.projectInfo?.customerCategory || '常规客户'}</AtTag>
                            </View>
                        </View>
                ...
                <View className="px-4 space-y-4">
                    ...
                    <DetailSection title="基本信息" iconColor="bg-amber-400">
                        ...
                        <InfoRow label="联系电话" value={info.contractPhone} isLink color="text-blue-600"
                            onClick={() => Taro.makePhoneCall({ phoneNumber: info.contractPhone })} />
                        <InfoRow label="职位" value={info.jobTitle || '经理'} />
                        <InfoRow label="籍贯" value={info.originAddr || '暂无'} border={false} />
                    </DetailSection>

                    {/* Section: 项目基本信息 */}
                    <DetailSection title="项目基本信息" iconColor="bg-blue-400">
                        <InfoRow label="主营项目" value={info.projectInfo?.projectName || '暂无'} />
                        <InfoRow label="区域地址" value={info.projectInfo?.region?.join(' / ') || info.addr} />
                        <InfoRow label="详细地址" value={info.projectInfo?.detailAddr || info.addr} />
                        <InfoRow label="合作类型" value={info.projectInfo?.cooperationType} />
                        <InfoRow label="客户类型" value={info.projectInfo?.customerType} />
                        <InfoRow label="客户来源" value={info.projectInfo?.customerSource} border={false} />
                    </DetailSection>
                ...
                {/* 底部操作栏（可选） */}
                <View className="fixed bottom-0 left-0 w-full p-4 bg-white/80 backdrop-blur-md flex gap-4">
...
/** 详情行组件 */
const InfoRow = ({ label, value, isLink = false, color = "text-gray-700", border = true, onClick = () => { } }) => (
    <View
    ...
    >
        ...
        <View className="flex-1 flex items-center justify-end">
            <Text className={`text-sm font-medium text-right ${color}`}>{value || '-'}</Text>
