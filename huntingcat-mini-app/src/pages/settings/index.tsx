...
const SettingsPage: React.FC = () => {
  ...
  return (
    <View className="min-h-screen bg-gray-100 pb-10">

      {/* 循环渲染分组 */}
      {menuGroups.map((group, gIndex) => (
        <View key={gIndex} className="mb-4 bg-white border-t border-b border-gray-200">
          {group.map((item, iIndex) => (
            <View 
              key={item.id}
              onClick={() => item.type !== 'switch' && handleNavigate(item.id)}
            ...
            >
              ...
              <View className="flex flex-row items-center">
                ...
                {item.type === 'text' && (
                  <Text className="text-sm text-gray-400 mr-1">{item.value}</Text>
                )}
      ...
      <View className="mt-8 px-4">
        <View 
          ...
          onClick={() => Taro.showModal({ title: '提示', content: '确定退出登录吗？' })}
        >
